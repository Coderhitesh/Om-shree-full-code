...
exports.userDetails = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId).populate('RefrealUserIds');
    ...
    try {
        ...
        const emailOptions = {
            ...
            message: `
                ...
                <body>
                    <p>Your OTP for Become Affiliate is: <strong>${OTP}</strong></p>
                    <p>Please use this OTP within 10 minutes to Become a Affiliate.</p>
                </body>
                </html>
            `
        };
...
exports.verifyAlliateOtp = async (req, res) => {
    try {
        ...
        user.Role = "Affiliate";
        await user.save()
        res.status(200).json({ success: true, message: 'You are now an Affiliate',user:user })
